 ...
 *   0=Monday, 1=Tuesday, 2=Wednesday, 3=Thursday, 4=Friday, 5=Sunday, 6=Saturday
 *
...
struct ConstraintSettings {
    ...
    bool ruleInstructorDayGap        = true;  // instructor must skip ≥1 day between teaching days
    bool ruleNoSameSubjectConsecDays = true;  // same course-batch pair on back-to-back days
    bool ruleRespectMaxWeeklyHours   = true;  // honour instructor.maxLimitHours
    bool ruleMaxConsecHoursEnabled   = true;  // limit consecutive teaching hours per day
    int  ruleMaxConsecHoursPerDay    = 3;     // max consecutive hours before mandatory break
    bool ruleEnforceSubjectLock      = true;  // always true — instructor must be qualified
    ...
    int weeklyAvailableMinutes() const {
        ...
        for (int i = 0; i < 7; ++i)
            if (workingDays[i]) ++workingDayCount;
